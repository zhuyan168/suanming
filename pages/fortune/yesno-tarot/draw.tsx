...
const tarotCards = [
  ...
  { id: 24, name: 'Three of Cups', image: '/assets/tarot-webp-optimized/minor_arcana_cups_3.webp', upright: '友谊庆祝、合作、社交', reversed: '孤立、过度放纵、三角关系', keywords: ['庆祝', '友谊', '创造'] },
...
// 保存抽牌结果到 localStorage
const saveYesNoTarotDraw = (data: YesNoTarotDraw) => {
  if (typeof window === 'undefined') return;
  localStorage.setItem(STORAGE_KEY_DRAW, JSON.stringify(data));
};

// 从 localStorage 读取抽牌结果
const loadYesNoTarotDraw = (): YesNoTarotDraw | null => {
  if (typeof window === 'undefined') return null;
  try {
    const data = localStorage.getItem(STORAGE_KEY_DRAW);
    if (!data) return null;
    return JSON.parse(data);
  } catch (error) {
    console.error('Failed to load yes/no tarot draw:', error);
    return null;
  }
};

// 从 localStorage 读取用户问题
const loadYesNoTarotQuestion = (): string => {
  if (typeof window === 'undefined') return '';
  try {
    return localStorage.getItem(STORAGE_KEY_QUESTION) || '';
  } catch (error) {

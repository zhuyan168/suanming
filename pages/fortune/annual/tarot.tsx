...
import ThirteenCardSlots from '../../../components/fortune/ThirteenCardSlots';
...
// localStorage 存取函数
const getYearAheadKey = (year: number): string => {
...
const loadYearAheadResult = (year: number): YearAheadResult | null => {
...
export default function TarotAnnualReading() {
  ...
  // 13张卡槽的状态
  const [selectedCards, setSelectedCards] = useState<(ShuffledTarotCard | null)[]>(
    Array(13).fill(null)
  );
  ...
  // 初始化：检查本年度是否已经抽过牌
  useEffect(() => {
    if (typeof window === 'undefined' || !currentYear) return;

    const stored = loadYearAheadResult(currentYear);
    if (stored && stored.cards.length === 13) {
      const validatedCards = stored.cards.map(card => {
      ...
      setSavedResult({ ...stored, cards: validatedCards });
  ...
  const drawCard = async (slotIndex: number) => {
    ...
    if (updatedCardCount === 13 && currentYear && typeof window !== 'undefined') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('✅ 13张牌抽取完成，已保存到localStorage');
      }
    }
  };

  const handleViewResult = () => {
    router.push('/fortune/annual/tarot/result');
  };
  ...
  return (
    <>
      <Head>
        <title>塔罗年度运势 - Mystic Insights</title>
            ...
            <div className="mx-auto max-w-7xl">
              <div className="text-center mb-12">
                ...
                <h1 className="text-4xl sm:text-5xl font-black leading-tight tracking-tight mb-4">
                  {hasDrawnThisYear ? `${currentYear}年运势已抽取` : `${currentYear} 年度运势`}
                </h1>
                <p className="text-white/70 text-lg max-w-2xl mx-auto">
                  {hasDrawnThisYear 
                    ? '你已抽取本年度运势牌阵，点击下方按钮查看详细解析。' 
                    : '这个月历牌阵为你展示来年每个月的运势细节。从底部开始顺时针抽取，中央为年度主题牌。'}
                </p>
              </div>
              ...
              <AnimatePresence>
                {showCards && !hasDrawnThisYear && (
                  ...
                  >
                    ...
                    <div className="mb-6">
                      <div className="text-center text-white/60 text-base font-medium">
                        <p>已抽牌：<span className="text-primary font-bold text-lg">{selectedCards.filter(c => c !== null).length}</span> / 13</p>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div className="text-center mt-10">
                    <motion.button
                      ...
                      className="px-10 py-4 rounded-xl bg-primary text-white font-bold text-lg transition-all duration-300 hover:bg-primary/90 hover:shadow-[0_0_20px_rgba(127,19,236,0.5)]"
                    >
                      查看运势
                    </motion.button>
                  </div>
              ...
              {!hasDrawnThisYear && canStartReading && (
                ...
                >
                  ...
                  <p className="text-white/50 text-sm mt-4">🎉 13张牌已抽取完成</p>
                </motion.div>
              )}
            </div>
          </main>

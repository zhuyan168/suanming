...
const generateSessionId = (): string => {
  return `interview-exam-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
};

const STORAGE_KEY = 'interview_exam_result';

interface InterviewExamResult {
  sessionId: string;
  timestamp: number;
...
const loadResult = (): InterviewExamResult | null => {
...
export default function InterviewExamDraw() {
  ...
  const drawCard = async (slotIndex: number) => {
    ...
    if (updatedCardCount === 5) {
      const result: InterviewExamResult = {
        sessionId,
        timestamp: Date.now(),
  ...
  const handleViewResult = () => {
    router.push('/themed-readings/career-study/interview-exam-key-reminders/reading');
  };

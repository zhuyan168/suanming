...
const generateSessionId = (): string => {
  return `offer-decision-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
};
...
const loadResult = (): OfferDecisionResult | null => {
...
export default function OfferDecisionDraw() {
  ...
  const drawCard = async (slotIndex: number) => {
    ...
    if (updatedCardCount === 6) {
      const result: OfferDecisionResult = {
        sessionId,
        timestamp: Date.now(),
          ...
          <div className="mx-auto max-w-7xl">
            ...
            {hasDrawn && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mt-10">
                <button
                  onClick={() => {
                    router.push('/themed-readings/career-study/offer-decision/reading');
                  }}

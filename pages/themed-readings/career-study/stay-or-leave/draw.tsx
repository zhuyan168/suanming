...
const generateSessionId = (): string => {
  return `stay-or-leave-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
};

const STORAGE_KEY = 'career_spread_should_i_stay_v1';
...
const SLOT_CONFIG = [
  { id: "p1", name: "你目前的职业状态", meaning: "你目前的职业状态" },
  { id: "p2", name: "这份工作的优势", meaning: "这份工作的优势" },
  { id: "p3", name: "这份工作的劣势", meaning: "这份工作的劣势" },
  { id: "p4", name: "领导/上司对你的看法", meaning: "领导/上司对你的看法" },
  { id: "p5", name: "同事/下属对你的看法", meaning: "同事/下属对你的看法" },
...
const loadResult = (): StayOrLeaveResult | null => {
  if (typeof window === 'undefined') return null;
  try {
    // 优先从 sessionStorage 取，双保险
...
export default function StayOrLeaveDraw() {
  ...
  const drawCard = async (slotIndex: number) => {
    ...
    if (updatedCardCount === 7) {
      const result: StayOrLeaveResult = {
        sessionId,
        timestamp: Date.now(),
  ...
  const handleViewResult = () => {
    router.push(`/themed-readings/career-study/stay-or-leave/reading?key=${STORAGE_KEY}`);
  };

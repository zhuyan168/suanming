...
export default function HorseshoeReadingPage() {
  ...
  // 自动生成解读（仅会员）
  useEffect(() => {
    if (result && result.cards.length === 7 && !reading && !loading && !error && isMember) {
      generateReading();
    }
  }, [result, isMember]);
      ...
      <>
        ...
        <div className="min-h-screen bg-[#0f0f23] text-white">
          ...
          <header className="sticky top-0 z-50 flex items-center justify-between border-b border-white/10 px-4 sm:px-8 md:px-16 lg:px-24 py-3 bg-[#0f0f23]/80 backdrop-blur-sm">
            <button
              onClick={() => router.push('/reading/general/horseshoe/reveal')}
            ...
            <div className="mx-auto max-w-2xl">
              ...
              >
                ...
                <p className="text-white/70 text-lg leading-relaxed mb-8">
                  马蹄铁牌阵的深度解读是会员专属功能。开通会员后，你将获得由 AI 生成的完整解读，包括整体分析、逐张牌解析和行动建议。
                </p>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-8 text-left">
                  ...
                  <ul className="space-y-2 text-white/70 text-sm">
                    <li className="flex items-start gap-2">
                      ...
                      <span>整体解读：综合分析 7 张牌的能量趋势</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-primary mt-1">•</span>
                      <span>逐张解析：每张牌在其牌位上的具体含义</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-primary mt-1">•</span>
                      <span>现实提醒：可执行的行动建议与情绪安抚</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-primary mt-1">•</span>

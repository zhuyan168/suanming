...
export default function CelticCrossReadingPage() {
  ...
  const texts = {
    ...
    noQuestion: isEn ? 'No question provided — we\'ll give you a general reading.' : '你没有填写具体问题，我们将为你做一次通用解读',
    overallReading: isEn ? 'Overall Guidance' : '整体解读',
    upright: isEn ? 'Upright' : '正位',
    reversed: isEn ? 'Reversed' : '逆位',
    actionAdvice: isEn ? 'Action Advice' : '行动建议',
                            ...
                            <div className="relative w-24 h-40 mb-3 group overflow-hidden rounded-lg">
                              {cardData?.image ? (
                                ...
                                >
                                  <img
                                    src={cardData.image}
                                    alt={isEn ? cardData.name : getChineseCardName(cardData.name)}
                          ...
                          <div className="flex-1 space-y-3">
                            <div className="flex items-center gap-3">
                              <span
                                ...
                                style={{ color: '#a855f7' }}
                              >
                                {isEn ? (cardData?.name || '') : getChineseCardName(cardData?.name || '')}
                              </span>
                              <span

...
interface MoneyBlocksSlotsProps {
...
export default function MoneyBlocksSlots({ 
  cards, 
  isAnimating,
  ...
  const renderCard = (index: number) => {
    ...
    return (
      <div key={index} className="flex flex-col items-center gap-2 w-full">
        <AnimatePresence mode="wait">
          {card ? (
            ...
            <motion.div
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0.9 }}
              className="w-20 h-32 sm:w-28 sm:h-42 md:w-32 md:h-48 rounded-xl border-2 border-dashed border-white/20 flex items-center justify-center bg-white/5"
            >
        ...
        {/* Position Info */}
        <div className="text-center mt-1 flex flex-col gap-1 px-1">
          <p className="text-primary text-[9px] sm:text-[10px] md:text-xs font-bold tracking-tight uppercase leading-tight h-6 flex items-center justify-center">
            {config?.name}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="money-blocks-slots w-full flex flex-col items-center py-4 sm:py-8">
      <div className="grid grid-cols-6 gap-x-2 gap-y-6 sm:gap-x-4 sm:gap-y-8 w-full max-w-4xl justify-items-center px-2">
        {/* Row 1: Cards 1, 2, 3 */}

const zh = {
  ...
  question: {
    ...
    h1: '二选一牌阵',
    tagline: '两个选择摆在面前，怎么选都觉得有得有失？可以用这个牌阵比较一下。',
    detail: '五张牌会先看看你现在为什么难以决定，再分别分析选择 A 和选择 B 后可能经历的状态，以及两条路接下来的发展趋势。',
    scope: '它适合两个已经比较明确的选项，比如两份工作、留下或离开、主动或等待。牌阵不会替你做决定，而是把两条路分别摊开，帮你判断哪一种更符合现在的自己。',
...
const en: typeof zh = {
  ...
  question: {
    ...
    tagline: 'Two options in front of you, and both seem to come with something to gain and something to lose? This spread can help you compare them.',
    detail: 'The five cards first explore why the decision feels difficult, then look separately at what you may experience with Option A and Option B, along with how each path may develop.',
    scope: 'This spread works best when you already have two clear options, such as two jobs, staying or leaving, or acting versus waiting. It will not make the decision for you, but it lays out both paths so you can see which one fits you better right now.',
